...
public abstract class MondrianOlap4jConnection implements OlapConnection {
  static {
    Bug.olap4jUpgrade(
      "Make this class package-protected when we upgrade to olap4j 2.0. "
        + "The setRoleNames method will then be available through the olap4j API" );
  }

  private static final Logger LOGGER = LogManager.getLogger( MondrianOlap4jConnection.class );
   ...
   */
  private final RolapConnection mondrianConnection;
  ...
  MondrianOlap4jConnection( Factory factory, MondrianOlap4jDriver driver, String url, Properties info )
    ...
    this.olap4jDatabaseMetaData = factory.newDatabaseMetaData( this, mondrianConnection );
    ...
    List<Map<String, Object>> dbpropsMaps = mondrianServer.getDatabases( mondrianConnection );
    ...
    for ( String catalogName : catalogFinder.getCatalogNames( mondrianConnection ) ) {
      ...
      try {
        rolapSchemas = catalogFinder.getRolapSchemas( mondrianConnection, catalogName );
      } catch ( Exception e ) {
        LOGGER.warn( String.format( "Can't get Rolap Schemas for catalog: %s. Skipping...", catalogName ), e );
        continue;
      }

      olap4jCatalogs.add( new MondrianOlap4jCatalog( olap4jDatabaseMetaData, catalogName, database, rolapSchemas ) );
    }
  ...
  public OlapStatement createStatement() {
    final MondrianOlap4jStatement statement = factory.newStatement( this );
  ...
  synchronized MondrianOlap4jSchema toOlap4j( mondrian.olap.Schema schema ) {
    MondrianOlap4jSchema mondrianOlap4jSchema = schemaMap.get( schema );
  ...
  Type toOlap4j( mondrian.olap.type.Type type ) {
    if ( type instanceof mondrian.olap.type.BooleanType ) {
      ...
      final mondrian.olap.Cube mondrianCube = ( (mondrian.olap.type.CubeType) type ).getCube();
  ...
  MondrianOlap4jMember toOlap4j( mondrian.olap.Member member ) {
    ...
    if ( member instanceof RolapMeasure ) {
      ...
      return new MondrianOlap4jMeasure( toOlap4j( member.getDimension().getSchema() ), measure );
    }
  ...
  MondrianOlap4jNamedSet toOlap4j( mondrian.olap.Cube cube, mondrian.olap.NamedSet namedSet ) {
    if ( namedSet == null ) {
      return null;
    }
  ...
  public void setRoleNames( List<String> roleNames ) throws OlapException {
    ...
    for ( String name : roleNames ) {
      ...
      final Role role = connection1.getSchema().lookupRole( name );
      if ( role == null ) {
        throw helper.createException( "Unknown role '" + name + "'" );
      }
      roleList.add( role );
    }
  ...
  static class Helper {
    ...
    OlapException createException( String msg, Throwable cause ) {
      ...
      assert !mondrian.util.Bug.olap4jUpgrade( "use OlapException(String, String, Throwable) ctor" );
  ...
  private static class MondrianToOlap4jNodeConverter {
    ...
    public SelectNode toOlap4j( Query query ) {
      ...
      return new SelectNode(
        ...
        new CubeNode( null, olap4jConnection.toOlap4j( query.getCube() ) ),
        query.getSlicerAxis() == null ? null : toOlap4j( query.getSlicerAxis() ),
        list );
    }
    ...
    private AxisNode toOlap4j( QueryAxis axis ) {
      return new AxisNode(
        null,
        axis.isNonEmpty(),
        Axis.Factory.forOrdinal( axis.getAxisOrdinal().logicalOrdinal() ),
    ...
    private ParseTreeNode toOlap4j( Exp exp ) {
      ...
      if ( exp instanceof DimensionExpr ) {
        ...
        return new DimensionNode( null, olap4jConnection.toOlap4j( dimensionExpr.getDimension() ) );
      }
      if ( exp instanceof HierarchyExpr ) {
        HierarchyExpr hierarchyExpr = (HierarchyExpr) exp;
        return new HierarchyNode( null, olap4jConnection.toOlap4j( hierarchyExpr.getHierarchy() ) );
      }
      ...
      if ( exp instanceof MemberExpr ) {
        ...
        return new MemberNode( null, olap4jConnection.toOlap4j( memberExpr.getMember() ) );
      }
        ...
        if ( formula.isMember() ) {
          ...
          for ( Object child : formula.getChildren() ) {
            if ( child instanceof MemberProperty ) {
              ...
              memberPropertyList.add( new PropertyValueNode(
                null,
