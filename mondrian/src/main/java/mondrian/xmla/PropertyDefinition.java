...
public enum PropertyDefinition {
  AxisFormat(
    ...
    "Determines the format used within an MDDataSet result set to describe the axes of the multidimensional dataset. "
      + "This property can have the values listed in the following table: TupleFormat (default), ClusterFormat, "
      + "CustomFormat." ),
  ...
  BeginRange(
    ...
    "-1",
    XmlaConstants.Method.EXECUTE,
    "Contains a zero-based integer value corresponding to a CellOrdinal attribute value. (The CellOrdinal attribute "
      + "is part of the Cell element in the CellData section of MDDataSet.)\n"
      + "Used together with the EndRange property, the client application can use this property to restrict an OLAP "
      + "dataset returned by a command to a specific range of cells. If -1 is specified, all cells up to the cell "
      + "specified in the EndRange property are returned.\n"
  ...
  Catalog(
    ...
    "When establishing a session with an Analysis Services instance to send an XMLA command, this property is "
      + "equivalent to the OLE DB property, DBPROP_INIT_CATALOG.\n"
      + "When you set this property during a session to change the current database for the session, this property is"
      + " equivalent to the OLE DB property, DBPROP_CURRENTCATALOG.\n"
  ...
  Content(
    ...
    "An enumerator that specifies what type of data is returned in the result set.\n"
      + "None: Allows the structure of the command to be verified, but not executed. Analogous to using Prepare to "
      + "check syntax, and so on.\n"
      + "Schema: Contains the XML schema (which indicates column information, and so on) that relates to the "
      + "requested query.\n"
  ...
  Cube(
    ...
    "",
    XmlaConstants.Method.EXECUTE,
    "The cube context for the Command parameter. If the command contains a cube name (such as an MDX FROM clause) the"
      + " setting of this property is ignored." ),
  ...
  EndRange(
    ...
    "-1",
    XmlaConstants.Method.EXECUTE,
    "An integer value corresponding to a CellOrdinal used to restrict an MDDataSet returned by a command to a "
      + "specific range of cells. Used in conjunction with the BeginRange property. If unspecified, all cells are "
      + "returned in the rowset. The value -1 means unspecified." ),
  ...
  Format(
    ...
    "Enumerator that determines the format of the returned result set. Values include:\n"
      + "Tabular: a flat or hierarchical rowset. Similar to the XML RAW format in SQL. The Format property should be "
      ...
      + "Native: The client does not request a specific format, so the provider may return the format  appropriate to"
      + " the query. (The actual result type is identified by namespace of the result.)" ),
  ...
  LocaleIdentifier(
    ...
    "Use this to read or set the numeric locale identifier for this request. The default is provider-specific.\n"
      + "For the complete hexadecimal list of language identifiers, search on \"Language Identifiers\" in the MSDN "
      + "Library at https://www.msdn.microsoft.com.\n"
      + "As an extension to the XMLA standard, Mondrian also allows locale codes as specified by ISO-639 and ISO-3166"
      + " and as used by Java; for example 'en-US'.\n" ),
  ...
  MDXSupport(
    ...
    "Core",
    XmlaConstants.Method.DISCOVER,
    "Enumeration that describes the degree of MDX support. At initial release Core is the only value in the "
      + "enumeration. In future releases, other values will be defined for this enumeration." ),
  ...
  Password(
    ...
    "",
    XmlaConstants.Method.DISCOVER_AND_EXECUTE,
    "This property is deprecated in XMLA 1.1. To support legacy applications, the provider accepts but ignores the "
  ...
  DbpropMsmdMDXCompatibility(
    ...
    "0",
    XmlaConstants.Method.DISCOVER_AND_EXECUTE,
    "An enumeration value that determines how placeholder members in a ragged or unbalanced hierarchy are treated." ),
  ...
  StateSupport(
    ...
    "None",
    XmlaConstants.Method.DISCOVER,
    "Property that specifies the degree of support in the provider for state. For information about state in XML for "
      + "Analysis, see \"Support for Statefulness in XML for Analysis.\" Minimum enumeration values are as follows:\n"
  ...
  UserName(
    ...
    "Returns the UserName the server associates with the command.\n"
      + "This property is deprecated as writeable in XMLA 1.1. To support legacy applications, servers accept but "
      + "ignore the password setting when it is used with the Execute method." ),
